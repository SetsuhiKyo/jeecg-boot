...
public class LoginController {
	...
	public Result<Object> customerLogout(HttpServletRequest request,HttpServletResponse response) {
		//用户退出逻辑
		String token = request.getHeader(CommonConstant.X_ACCESS_TOKEN);
		if(oConvertUtils.isEmpty(token)) {
			return Result.error("退出登录失败！");
		...
		queryWrapper.eq(YyCustomer::getCustomerName,username);
		YyCustomer customer = yyCustomerService.getOne(queryWrapper);
		if(customer!=null) {
			//update-begin--Author:wangshuai  Date:20200714  for：登出日志没有记录人员
//			baseCommonService.addLog("顾客: "+customer.getEmail()+",退出成功！", CommonConstant.LOG_TYPE_1, null,customer);
			//update-end--Author:wangshuai  Date:20200714  for：登出日志没有记录人员
			log.info(" 顾客:  "+customer.getEmail()+",退出成功！ ");
			//清空用户登录Token缓存
			redisUtil.del(CommonConstant.PREFIX_USER_TOKEN + token);
			//清空用户登录Shiro权限缓存
			redisUtil.del(CommonConstant.PREFIX_USER_SHIRO_CACHE + customer.getId());
			//清空用户的缓存信息（包括部门信息），例如sys:cache:user::<username>
			redisUtil.del(String.format("%s::%s", CacheConstant.SYS_USERS_CACHE, customer.getCustomerName()));
			//调用shiro的logout
			SecurityUtils.getSubject().logout();
			return Result.ok("退出登录成功！");
		}else {
	...
	public Result<JSONObject> customerLogin(@RequestBody SysLoginModel sysLoginModel) throws Exception {
		...
		//2. 校验用户名或密码是否正确
		String userpassword = PasswordUtil.encrypt(username, password, customer.getCustomerSalt());
		String syspassword = customer.getCustomerPwd();
		if (!syspassword.equals(userpassword)) {
			//update-begin-author:taoyan date:2022-11-7 for: issues/4109 平台用户登录失败锁定用户
			addLoginFailOvertimes(username);
			//update-end-author:taoyan date:2022-11-7 for: issues/4109 平台用户登录失败锁定用户
		...
		//5. 设置登录用户信息
		YyCustomer rtnCustomer = new YyCustomer();
		...
		result.setSuccess(true);
		result.setCode(200);
		baseCommonService.addLog("顾客名: " + username + ",登录成功！", CommonConstant.LOG_TYPE_1, null);
		return result;
	}
}