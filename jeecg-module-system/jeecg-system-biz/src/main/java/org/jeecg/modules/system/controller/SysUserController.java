...
public class SysUserController {
    ...
    public Result<IPage<SysUser>> queryAllPageList(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                                   @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        if (req.getParameterMap().containsKey("roleId")){
            return userRoleList(pageNo,pageSize,req);
        } else {
            QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(user, req.getParameterMap());
            return sysUserService.queryPageList(req, queryWrapper, pageSize, pageNo);
        }
    ...
    public Result<IPage<SysUser>> userRoleList(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        IPage<SysUser> pageList = sysUserService.getUserByRoleId(page,roleId,username,email,phone);
        result.setSuccess(true);
...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    @IgnoreAuth
    public Result<JSONObject> customerRegister(@RequestBody JSONObject jsonObject) {
        Result<JSONObject> result = new Result<JSONObject>();
        String email = jsonObject.getString("email");
        String username = jsonObject.getString("username");
        String password = jsonObject.getString("password");
...
// TODO           user.setPhone("");
            ...
            user.setDelFlag(CommonConstant.DEL_FLAG_0);
            user.setActivitiSync(CommonConstant.ACT_SYNC_1);
            sysUserService.addUserWithRole(user,"1937767948390785025");//顾客角色 customer
            result.success("用户注册成功");
        } catch (Exception e) {
            result.error500("用户注册失败。请联系客服人员帮您解决。");
        }
        return result;
    }
    ...
    @IgnoreAuth
    public Result<JSONObject> resetPwd(@RequestBody JSONObject jsonObject) {
        Result<JSONObject> result = new Result<JSONObject>();
        String email = jsonObject.getString("email");
        SysUser sysUser3;

        if(oConvertUtils.isNotEmpty(email)){
            sysUser3 = sysUserService.getUserByEmail(email);
            if (sysUser3 == null) {
                result.setMessage("邮箱没有被注册过，请输入你在注册时使用的邮箱地址");
                result.setSuccess(false);
                return result;
            }
        } else {
        ...
        try {
            ...
            sysUserService.changePassword(sysUser3);
            result.success("密码重置成功");
        } catch (Exception e) {
            result.error500("密码重置失败。请联系客服人员帮您解决。");
        }
        return result;
    }
