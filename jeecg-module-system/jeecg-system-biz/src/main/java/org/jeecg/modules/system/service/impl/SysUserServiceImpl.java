...
/*	@Override
     ...
     */
	@Override
	public IPage<SysUser> getUserByRoleId(Page<SysUser> page, String roleId, String username, String email, String phone) {
		//update-begin---author:wangshuai ---date:20230220  for：[QQYUN-3980]组织管理中 职位功能 职位表加租户id 加职位-用户关联表------------
		IPage<SysUser> userRoleList = userMapper.getUserByRoleId(page, roleId, username,email,phone);
...
//		}
	...
	public void checkUserAdminRejectDel(String userIds) {
		...
		query.eq(SysUser::getUsername,"Administrator");
		Long adminRoleCount = this.baseMapper.selectCount(query);
		//大于0说明存在管理员用户，不允许删除
		if(adminRoleCount>0){
			throw new JeecgBootException("超级管理员用户，不允许删除！");
		}
	}
